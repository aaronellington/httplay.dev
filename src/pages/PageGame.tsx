...
export function PageGame<State, Update>(props: GameProps<State, Update>) {
	const [endpoint, setEndpoint] = useState(
		`http://127.0.0.1:2690/${props.Game.Slug}`,
	)
...
function SectionInstructions<State, Update>(props: SubProps<State, Update>) {
	return (
		<Content>
			...
			<hr />
			<pre>{JSON.stringify(props.Game.ExampleUpdate(), null, 4)}</pre>
		</Content>
	)
}
...
function SectionConfig<State, Update>(props: SubProps<State, Update>) {
	const runStep = async () => {
		const tmpHistory = JSON.parse(JSON.stringify(props.history)) as History<
			State,
			Update
		>
		try {
			...
			try {
				...
				let result = props.Game.ApplyUpdate(
					"Player",
					tmpHistory.Result.State,
					update,
				)
	...
	return (
		<Content>
			...
			<Panel centered>
				<Button
					disabled={props.history.Result.Success !== null}
					onClick={() => {
						runStep()
					}}
				>
				...
				<p>{props.history.Result.Message}</p>
			</Panel>
		</Content>
	)
}
...
function SectionGameState<State, Update>(props: SubProps<State, Update>) {
	...
	useAutosizeTextArea(
		textAreaRef.current,
		JSON.stringify(props.history.Result, null, 4),
	)
	...
	return (
		<Content>
			...
			<textarea
				onKeyUp={(event: any) => {
					try {
						const x = JSON.parse(JSON.stringify(props.history)) as History<
							State,
							Update
						>
				...
				value={JSON.stringify(props.history.Result.State, null, 4)}
			/>
		</Content>
	)
}
			...
			{props.history.Steps.map((x) => {
				if (typeof x === "string") {
					return (
						<pre
							className={
								props.history.Result.Success === true
									? classes.player
									: classes.error
							}
						>
				...
				return (
					<pre
						className={x.Actor === "Player" ? classes.player : classes.computer}
					>
